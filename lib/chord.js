...
var createChord = function (chord, octave) {
  return chord instanceof Chord ? chord : new Chord(chord, octave);
};

var ChordChart = function (chords) {
  this.chords = _.map(chords, function (arr) {
    return { chord: createChord(arr[0]), type: {beats: arr[1]} };
  });

  this.midi = function (filename, options, callback) {
    return midi.output('chordChart', this, filename, options, callback);
  };
};
