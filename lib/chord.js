...
var optimizeScalePrecedence = function (scales, chord) {
  if (_.contains(['m', 'm7', 'm9', 'm11', 'm13'], chord.symbol)) {
    scales = _.filter(scales, function (scale) {
      return scale.id !== 'bebop_dorian';
    });
  }
  if (_.contains(['7', '9', '11', '13', 'M7', 'M9', 'M11', 'M13'], chord.symbol)) {
    scales = _.filter(scales, function (scale) {
      return scale.id !== 'bebop_dominant';
    });
  }
  if (_.contains(['7', '7#9', '7+9', '7#11', '7+11'], chord.symbol)) {
    scales.splice(2, 0, scale.create(chord.root, 'blues'));
  }
  return scales;
};
...
Chord.prototype.scales = function () {
  ...
  var scales = _.map(scale.precedence, function (name) {
    return scale.create(chord.root, name);
  });

  scales = _.filter(scales, function (scale) {
    return _.every(chord.chord, function (note) {
      return _.some(scale.scale, function (scaleNote) {
        return scaleNote.enharmonic(note);
      });
    });
  });

  return optimizeScalePrecedence(scales, chord);
};
...
Chord.prototype.octave = function (octave) {
...
module.exports.create = function (chord, octave) {
