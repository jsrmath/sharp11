...
var overChart = function (chart, settings) {
  ...
  if (settings.rests instanceof Array) {
    restDelta = (settings.rests[1] - settings.rests[0]) / beats;
    settings.rests = settings.rests[0];
  }
  if (settings.rhythmicVariety instanceof Array) {
    rhythmDelta = (settings.rhythmicVariety[1] - settings.rhythmicVariety[0]) / beats;
    settings.rhythmicVariety = settings.rhythmicVariety[0];
  }
...
var Improvisor = function (settings) {
  var defaultSettings = {
    ...
    changeDir: 0.5, // Likelihood of changing chromatic direction, number (0 to 1)
    jumpiness: 0.5, // Likelihood of jumping intervals, number (0 to 1) or range
    rests: 0.5, // Likelihood of a note being a rest (previous note is sustained), number (0 to 1)
    rhythmicVariety: 0.5, // Likelihood of rhythmic variety, number (0 to 1) or range
...
var ImprovChart = function (chart) {
  ...
  this.midi = function (filename, options) {
