...
var dominantChords = ['IIIx', 'bIIIx', 'Vx', 'bVIIx', 'IVm'];
...
JzA.prototype.analyze = function (symbols) {
  ...
  var pathways = _.map(getPossibleInitialStates(this, symbols), function (state) {
    return [state];
  });
  var oldPathways;
  var nextStates;
...
JzA.prototype.findFailurePoint = function (symbols) {
  ...
  for (i = 1; i < symbols.length; i += 1) {
    ...
    currentStates = _.chain(currentStates)
      .invoke('getTransitionStatesBySymbol', symbols[i])
      .flatten()
      .uniq()
      .value();
...
var addTonicization = function (jza) {
  ...
  _.each(majorSevenths, function (t) {
    var vState = jza.getStateWithNameAndTransition('V / ' + t.symbol, t.to);
    var iiState = jza.getStateWithNameAndTransition('ii / ' + t.symbol, vState);
...
var addAppliedChords = function (jza) {
  ...
  _.each(chords, function (t) {
    var vState = jza.getStateWithNameAndTransition('V / ' + t.symbol, t.to);
    vState.addSource(t.symbol.transpose('P5').withQuality('x'), t.from);
    vState.addTransition(t.symbol, t.to);
  });
};
...
var addChromaticApproachingChords = function (jza) {
  ...
  _.each(chords, function (t) {
    var viiState = jza.getStateWithNameAndTransition('Chromatic approaching ' + t.symbol, t.to);
    viiState.addSource(t.symbol.transpose('M7').withQuality('x'), t.from);
...
var addDiminishedChords = function (jza) {
  ...
  _.each(jza.getTransitionsByQuality('m'), function (t) {
    var diminishedState = jza.getStateWithNameAndTransition('Diminished approaching ' + t.symbol, t.to);
    diminishedState.addSource(t.symbol.transpose('m2').withQuality('o'), t.from);
...
var addNeighborChords = function (jza) {
  // TODO: Do this less arbitrarily / add more neighbor chords
  var neighborCandidates = symbolsFromMehegan(['I', 'Im']);
  var neighborChords = symbolsFromMehegan(['IV', 'IVm', 'IVx']);

  _.each(neighborCandidates, function (neighborCandidate) {
    _.each(jza.getTransitionsBySymbol(neighborCandidate), function (t) {
      var preNeighborState = jza.addState(t.symbol + ' with neighbor', false);
      var neighborState = jza.addState('Neighbor of ' + t.symbol, true);
      t.from.addTransition(t.symbol, preNeighborState);
      t.to.addSource(t.symbol, neighborState);
...
var addPassingChords = function (jza) {
  ...
  _.each(passingSequences, function (passingSequence) {
    ...
    _.each(jza.getTransitionsBySymbol(symbols[0]), function (t) {
      ...
      var prePassingState = jza.addState(t.to.name + ' with passing chord');
      var passingState = jza.addState('Passing chord');
