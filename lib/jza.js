...
JzState.prototype.addTransition = function (symbol, state) {
  ...
  if (!_.some(this.transitions, function (e) {
    ...
    this.transitions.push(new JzTransition(this, state, symbol));
  }
};
...
JzA.prototype.getPathways = function (symbols) {
  ...
  // Each pathway is a list of transitions
  // We don't have a transition for the state we start with, so we will store a faux transition
  // to our initial state make the algorithm simpler, and we'll remove it later
  ...
  _.each(_.rest(symbols), function (symbol) {
    ...
    _.each(oldPathways, function (pathway) {
      transitions = _.last(pathway).to.getTransitionsBySymbol(symbol);

      // If we're at a dead end, don't add this pathway
      // Otherwise, add all possible pathways we can take
      if (transitions.length) {
        _.each(transitions, function (transitions) {
          pathways.push(pathway.concat([transitions]));
        });
      }
    });
  });
  ...
  return _.chain(pathways)
    ...
    .filter(function (p) {
      return _.last(p).to.isEnd;
    })
    // Remove faux initial transition
    .map(function (p) {
      return _.rest(p);
    })
    .value();
...
JzA.prototype.analyze = function (symbols) {
  return _.map(this.getPathways(symbols), function (pathway) {
    if (!pathway.length) return [];

    // To go from a list of transitions to a list of states
    // we take all the syncs and append the initial source to the beginning
    return [pathway[0].from].concat(_.pluck(pathway, 'to'));
  });
};
