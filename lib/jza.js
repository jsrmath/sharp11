...
// Keep a mapping of Mehegan strings to corresponding symbol objects so that we don't waste time creating new ones
var symbolCache = {};
...
// Given a symbol, return the symbol; given a Mehegan string, return it as a symbol
var ensureSymbol = function (sym) {
  if (sym instanceof Symbol) return sym;

  // Try to retrieve symbol from cache, creating a new symbol if it's not found
  if (!symbolCache[sym]) symbolCache[sym] = symbolFromMehegan(sym);
  return symbolCache[sym];
};

var ensureSymbols = _.partial(_.map, _, ensureSymbol);
...
var JzTransition = function (from, to, symbol, count) {
  this.from = from;
  this.to = to;
  this.symbol = ensureSymbol(symbol);
  this.count = count || 0;
};
...
JzA.prototype.getStateProbabilitiesGivenSymbol = function (symbol) {
  var transitions = this.getTransitionsBySymbol(symbol);
  var states = {};

  // Sum up counts, treating states with the same name as the same
  _.each(transitions, function (t) {
    var state = t.to.name;

    if (t.count) {
      states[state] = states[state] ? states[state] + t.count : t.count;
    }
  });

  return getProbabilitiesAndSort(states, getTotalCountForTransitions(transitions));
};

var getSymbolKey = function (symbols, symbol) {
  return _.find(_.keys(symbols), function (s) {
    return symbol.eq(s);
  }) || symbol.toString();
};
...
JzA.prototype.getSymbolProbabilitiesGivenStateRegex = function (regex) {
  ...
  // Sum up counts, treating states with the same name as the same
  _.each(transitions, function (t) {
    var symbolKey = getSymbolKey(symbols, t.symbol);

    if (t.count) {
      symbols[symbolKey] = symbols[symbolKey] ? symbols[symbolKey] + t.count : t.count;
    }
  });

  return getProbabilitiesAndSort(symbols, getTotalCountForTransitions(transitions));
};
...
JzA.prototype.getTransitionProbabilitiesGivenStateRegex = function (regex, keyType) {
  ...
  _.each(transitions, function (t) {
    ...
    switch (keyType) {
      case 'symbol':
        transitionKey = getSymbolKey(transitionMap, t.symbol);
        break;
      case 'state':
        transitionKey = t.to.name;
        break;
      default:
        transitionKey = t.symbol.toString() + ': ' + t.to.name;
    }

    if (t.count) {
      transitionMap[transitionKey] = transitionMap[transitionKey] ? transitionMap[transitionKey] + t.count : t.count;
    }
  });

  return getProbabilitiesAndSort(transitionMap, getTotalCountForTransitions(transitions));
};
