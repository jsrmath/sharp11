...
// All the chords to the song with the first chord appended at the end
Jazz.prototype.fullChordListWithWrapAround = function () {
  var chords = this.fullChordList();
  return chords.concat(chords[0]);
};

Jazz.prototype.sectionChordLists = function () {
  return _.mapObject(this.content, function (val) {
    return _.pluck(val.content, 'chord');
  });
};

// All the chords in each section with the first chord of the next section appended at the end
Jazz.prototype.sectionChordListsWithWrapAround = function () {
