...
Symbol.prototype.transpose = function (interval, down) {
  var n = note.create('C').transpose(this.toInterval()).transpose(interval, down);
...
JzState.prototype.addTransition = function (symbol, state) {
  // Don't add transition if equivalent one already exists
  if (!_.some(this.transitions, function (t) {
    return t.symbol.eq(symbol) && t.state === state;
  })) {
...
JzState.prototype.addSource = function (symbol, state) {
  // Don't add source if equivalent one already exists
  if (!_.some(this.sources, function (s) {
    return s.symbol.eq(symbol) && s.state === state;
  })) {
...
var addTritoneSubstitutions = function (jza) {
  var dominantSevenths = _.filter(jza.getTransitions(), function (t) {
    return t.symbol.quality === 'x';
  });
