...
var addUnpackedChords = function (jza) {
  ...
  _.each(dominantSevenths, function (t) {
    ...
    var unpackedState = _.find(jza.getStatesByName('Unpacked ' + t.symbol), function (s) {
      return s.transitions[0].state === t.to;
    });

    // Otherwise, create a new one
    if (!unpackedState) unpackedState = jza.addState('Unpacked ' + t.symbol);
