...
var scaleConstructor = function (key, scaleName, descending) {
  var intervals;
  var scaleId;
...
var Scale = function () {
  return scaleConstructor.apply(this, arguments)
};
...
var TraversableScale = function (scale, current) {
  // Octave number of scale root
  var octave;

  // Scale object to be returned
  var scale;
  ...
  octave = current.lowerThan(scale.root.name) ? current.octave - 1 : current.octave;

  // Initialize scale in proper octave
  scaleConstructor.call(this, scale.root.name + octave, scale.name, scale.descending);

  this.current = current;
};

// Return a TraversableScale object with scale and given note
Scale.prototype.traverse = function (note) {
  return new TraversableScale(this, note);
};
