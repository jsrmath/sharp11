...
var makeVLQ = function (num) {
  ...
  arr = _.map(arr, function (val, i) {
    if (i !== arr.length - 1) return val + 128;
...
var makeHeaderChunk = function (numTracks) {
  var chunklen = padNumber(6, 4);
  var format = padNumber(1, 2);
...
var makeImprovNoteList = function (data) {
  ...
  return _.flatten(_.map(noteArr, function (arr) {
    return _.map(arr, function (note, i) {
      return { note: note, type: arr.length, pos: i };
    });
  }));
};
...
var makeImprovNoteEvents = function (data, options) {
  ...
  var events = _.reduce(noteList, function (arr, obj) {
    ...
    if (obj.type === 2 && obj.pos === 0) noteType = 'longEighth';
    if (obj.type === 2 && obj.pos === 1) noteType = 'shortEighth';
    if (obj.type === 3) noteType = 'triplet';
    if (obj.type === 4) noteType = 'sixteenth';

    time = noteLength(noteType, options);

    if (obj.note) {
      arr.push(makeNoteEvent(restBuffer, true, 0, noteValue(obj.note), defaultVelocity));
      arr.push(makeNoteEvent(time, false, 0, noteValue(obj.note), defaultVelocity));
      restBuffer = 0;
    }
    else {
      restBuffer += time;
...
var makeImprovChordEvents = function (data, options) {
  ...
  var events = _.reduce(chordList, function (arr, obj) {
    ...
    _.each(obj.chord.chord, function (note, i) {
      arr.push(makeNoteEvent(0, true, i + 1, noteValue(note), defaultVelocity));
    });

    _.each(obj.chord.chord, function (note, i) {
      arr.push(makeNoteEvent(i ? 0 : time, false, i + 1, noteValue(note), defaultVelocity));
    });
...
module.exports.output = function (type, data, filename, options) {
  ...
  options = options || {};
  _.each(defaultOptions, function (val, key) {
