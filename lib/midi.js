...
var makeHeaderChunk = function (format, numTracks) {
  ...
  var format = padNumber(format, 2); // Usually format 1 MIDI file (multuple overlayed tracks)
...
var noteLength = function (noteType, options) {
  ...
  // Initialize ticks with the number of beats in 
  var ticks = noteType.beats ? noteType.beats * ticksPerBeat : 0;

  // `noteType.type` can either be undefined, a string, or an array of strings
...
var makeImprovChordList = function (data) {
  return _.map(data, function (obj) {
    return { chord: obj.chord.octave(4), type: {beats: obj.notes.length} };
  });
};
...
var makeImprovChordEvents = function (data, options) {
  ...
  var events = _.reduce(chordList, function (arr, obj) {
    ...
    arr.push(makeChordEvent(0, true, 1, obj.chord.chord, options.chordVelocity)); // On
    arr.push(makeChordEvent(time, false, 1, obj.chord.chord, options.chordVelocity)); // Off
