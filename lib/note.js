...
Note.prototype.transpose = function (int, down) {
  ...
  if (note.octave) {
    if (down) {
      if (this.lowerThan(note)) note = createNote(note, note.octave - 1);
      if (int.number > 7) note = createNote(note, note.octave - 1);
    }
    else {
      if (this.higherThan(note)) note = createNote(note, note.octave + 1);
      if (int.number > 7) note = createNote(note, note.octave + 1);
    }
  }
...
module.exports.random = function (range) {
  ...
  do {
    ...
    note = createNote(note, random.integer(lower.octave, upper.octave));
