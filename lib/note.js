...
Note.prototype.lowerThan = function (note) {
  ...
  if (n1.octave && n2.octave) {
    if (n1.octave < n2.octave) return true;
    if (n1.octave > n2.octave) return false;
  }
...
module.exports.random = function (lower, upper) {
  var note;

  if (!isNote(lower)) lower = new Note(lower);
  if (!isNote(upper)) upper = new Note(upper);
  ...
  } while (note.lowerThan(lower) || note.higherThan(upper));
