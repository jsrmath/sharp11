...
var getDiatonicHalfSteps = function (number) {
  number = number % scale.length;
  return halfStepMap[scale[number - 1]];
};

var getHalfStepOffset = function (int) {
...
Note.prototype.getInterval = function (note) {
  ...
  diatonicHalfSteps = getDiatonicHalfSteps(number);
...
Note.prototype.transpose = function (int) {
  ...
  // Find the half step offset to the major or perfect interval
  halfStepOffset = getDiatonicHalfSteps(int.number) - halfSteps;

  // Alter it to get the interval we actually want
  halfStepOffset += getHalfStepOffset(int);
